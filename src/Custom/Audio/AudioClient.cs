...
public partial class AudioClient
{
    ...
    /// <summary>
    /// Streams a life-like, spoken audio recording of the input text as it is generated by the OpenAI API.
    /// </summary>
    /// <param name="text">The text to generate audio for.</param>
    /// <param name="voice">The voice to use in the generated audio.</param>
    /// <param name="options">The options to configure the audio generation.</param>
    /// <param name="onAudioChunk">A callback invoked for each audio chunk as it is received.</param>
    /// <param name="cancellationToken">A token that can be used to cancel this method call.</param>
    /// <returns>A Task representing the asynchronous operation.</returns>
    public virtual async Task GenerateSpeechStreamingAsync(string text, GeneratedSpeechVoice voice, SpeechGenerationOptions options, Action<byte[]> onAudioChunk, CancellationToken cancellationToken = default)
    {
        Argument.AssertNotNull(text, nameof(text));
        ...
        using PipelineMessage message = CreateCreateSpeechRequest(content, cancellationToken.ToRequestOptions());

        // Send the request and stream the response using the pipeline extension
        var response = await Pipeline.ProcessMessageAsync(message, cancellationToken.ToRequestOptions()).ConfigureAwait(false);
        if (response.IsError)
        {
